...
class SafeHavenCollector:
    ...
    def _gold_from_gold_api(self):
        """Source 1: gold-api.com — free, no key required, live spot price.
           Replaces the old metals.live integration, whose v1 endpoint no
           longer completes a TLS handshake (dead/retired host)."""
        try:
            r = requests.get('https://api.gold-api.com/price/XAU', timeout=10)
            if r.status_code == 200:
                price = float(r.json()['price'])
                if 1500 < price < 5000:   # sanity check — real gold range
                    print(f"    [OK] Gold via gold-api.com: ${price}")
                    return price
                else:
                    print(f"    [WARN] gold-api.com returned suspicious price: ${price}")
        except Exception as e:
            print(f"    [WARN] gold-api.com failed: {e}")
        return None
    ...
    def _gold_from_metals_live(self):
        """Source 3: metals.live — kept as a last-resort fallback in case
           it comes back online, but its v1 spot endpoint has been failing
           TLS handshakes (SNI not recognized), so treat it as likely dead."""
        try:
            r = requests.get('https://api.metals.live/v1/spot/gold', timeout=10)
            if r.status_code == 200:
                price = float(r.json()[0]['price'])
    ...
    def get_gold_price(self):
        ...
        NOTE: Alpha Vantage's XAU/USD via FX_DAILY was removed — it is not
        a supported symbol pair on that endpoint at any tier (confirmed via
        'Invalid API call' error, not a rate-limit/premium issue), so it
        could never succeed. gold-api.com is a free, keyless replacement."""
        print("  [GOLD] Trying 3 sources...")

        price = (
            self._gold_from_gold_api()   or
            self._gold_from_yfinance()   or
            self._gold_from_metals_live()
        )
